...
let ProductoService = class ProductoService {
    ...
    async getAll() {
        ...
        if (!list.length) {
            throw new common_1.NotFoundException(new message_dto_1.MessageDto('La Lista esta Vacia'));
        }
        return list;
    }
    async findById(id) {
        ...
        if (!producto) {
            throw new common_1.NotFoundException(new message_dto_1.MessageDto('Ese Producto No Existe'));
        }
        return producto;
    }
    ...
    async update(id, dto) {
        ...
        if (!producto)
            throw new common_1.BadRequestException(new message_dto_1.MessageDto('Ese Producto No Existe'));
        const exists = await this.findByNombre(dto.nombre);
